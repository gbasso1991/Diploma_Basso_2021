...
#%% Grafico las señales y sus ajustes
    ...
    plt.text(max(t_m),-10,'$R^2$ = {}'.format(R_2_m),
    ...
    plt.text(max(t_m),10,'Amplitud: %0.2f V '%amplitud_m,
             bbox=dict(alpha=0.9), ha='right',
             va='bottom')
...
#%% Fondo
    ...
    plt.ylabel('Amplitud de señal (V)')
    plt.legend(loc='upper left',framealpha=1.0)
    plt.text(max(t_f),-10,'$R^2$ = {}'.format(R_2_f),
             bbox=dict(alpha=0.9), ha='right',
             va='top')
    plt.text(0,-10,'Frecuencia: %0.2f kHz '%frecuencia_f_kHz,
             bbox=dict(alpha=0.9), ha='left',
             va='top')
    plt.text(max(t_f),10,'Amplitud: %0.2f V '%amplitud_f,
             bbox=dict(alpha=0.9), ha='right',
             va='bottom')
...
#%% Calibracion 
    ...
    plt.ylabel('Amplitud de señal(V)')
    plt.legend(loc='upper left',framealpha=1.0)
    plt.text(max(t_c),-10,'$R^2$ = {}'.format(R_2_c),
             bbox=dict(alpha=0.9), ha='right',
             va='top')
    plt.text(0,-10,'Frecuencia: %0.2f kHz '%frecuencia_c_kHz,
             bbox=dict(alpha=0.9), ha='left',
             va='top')   
    plt.text(max(t_c),10,'Amplitud: %0.2f V '%amplitud_c,
             bbox=dict(alpha=0.9), ha='right',
             va='bottom')
...
#%%   
    ...
    plt.plot(t_fm*1e6, y_f_1, ls=':',label='Ajuste Fondo')
    plt.plot(t_m_1*1e6, y_m_1,ls=':',label='Ajuste Muestra')
    plt.xlim(0,1e6/frecuencia_m)
    plt.grid()
...
#%% 
    ...
    plt.plot(t_fc*1e6, y_f_1, ls=':',label='Ajuste Ref. Fondo')
    plt.plot(t_c_1*1e6, y_c_1,ls=':',label='Ajuste Calibración')
    plt.xlim(0,1e6/frecuencia_c)
    plt.grid()
    plt.xlabel('Tiempo ($\mu$s)')
    plt.ylabel('Amplitud de señal (V)')
    plt.legend(loc='upper left',framealpha=1.0)
...
#%%    
    ...
    Reptito metodo para señal de calibracion.
    '''
...
#%% Graficos de la resta de señales
    ...
    plt.plot(t_m_1*1e6,Resta_m,'.-',label='Muestra$-$Fondo',lw=1)
    plt.legend(loc='best',framealpha=1.0)
    plt.xlabel('Tiempo ($\mu$s)')
    plt.ylabel('Amplitud de señal (V)')
...
#%% Calibracion - Fondo
    ...
    plt.plot(t_c_1*1e6,Resta_c,'.-',label='Calibración$-$Fondo',lw=1)
    plt.legend(loc='best',framealpha=1.0)
    plt.xlabel('Tiempo ($\mu$s)')
    plt.ylabel('Amplitud de señal (V)')
...
#%%     Controlo que el suavizado final sea satisfactorio
        #Muestra
        ax1 = plt.subplot(211)
        plt.plot(t_m_1*1e6,v_r_m_1/max(v_r_m_1), '-',label='Ref. de Muestra',lw=1)
        plt.plot(t_m_3*1e6,v_r_m_3/max(v_r_m_3), '-',label='Ref. de Muestra filtrada',lw=1)
        ...
        plt.plot(t_m_1*1e6,Resta_m/max(Resta_m),'-',label='Resta MF',linewidth=0.7)
        plt.plot(t_m_3*1e6,Resta_m_3/max(Resta_m_3),'-',
                 label='Resta MF filtrada',linewidth=0.7)        
        plt.legend(loc='lower left',framealpha=1.0)
        plt.xlabel('t ($\mu$s)')
...
#%%     Calibracion   
        ax3 = plt.subplot(211)
        plt.plot(t_c_1*1e6,v_r_c_1/max(v_r_c_1),label='Ref. de Calibración ',lw=1)
        plt.plot(t_c_3*1e6,v_r_c_3/max(v_r_c_3),label='Ref. de Calibración  filtrada',lw=1)
        plt.legend(loc='lower left',framealpha=1.0)
        plt.setp(ax3.get_xticklabels(),visible=False)
        plt.ylabel('u.a.')
        plt.title(filenombres_cal[i]+ ' - Quita de ruido por FFT: Ref. de calibración ')
        plt.grid()
        
        ax4 = plt.subplot(212,sharex=ax3)
        plt.plot(t_c_1*1e6,Resta_c/max(Resta_c),label='Resta C$-$F',lw=0.7)
        plt.plot(t_c_3*1e6,Resta_c_3/max(Resta_c_3),label='C$-$F filtrada',lw=0.7)
        plt.xlabel('Tiempo ($\mu$s)')
        plt.ylabel('u.a.')
        plt.legend(loc='lower left',framealpha=1.0)
...
#%%        
 ...
 #%%Grafico señal de referencia (campo) y señal de muestra.     
    ax5 = plt.subplot(211)
    plt.plot(t_m_4*1e6,v_r_m_4,label='Ref. de Muestra',lw=1)
    plt.legend(loc='upper left',framealpha=1.0)
    plt.ylabel('Amplitud de señal (V)')
    plt.title(filenombres_muestra[i][:-4] + ' - Señal de Referencia y de Muestra') 
    ...
    plt.xlabel('Tiempo ($\mu$s)')
    plt.ylabel('Señal (V)')
    plt.title(filenombres_muestra[i][:-4] + ' Señal de referencia y de Calibración')
    plt.grid() 
    
    ax8 = plt.subplot(212,sharex=ax7)
    plt.plot(t_c_4*1e6,Resta_c_4,label='Calibración ',lw=1,c='#ff7f0e')
    plt.legend(loc='upper left',framealpha=1.0)
    plt.xlim(0.5*1e6/frecuencia_c,2.5*1e6/frecuencia_c) #2 periodos
    plt.grid()
    plt.tight_layout()
...
#%% Señales de Referencias interpoladas
    ...
    plt.grid()
    plt.title(filenombres_muestra[i][:-4] + ' - Señal de Fondo: Muestra y calibración')
...
# =============================================================================
#%%    
    '''Exporto ciclos de histeresis en ascii: 
                    campo || magnetizacion_m || magnetizacion_m/max(magnetizacion_m)
    '''
   ...
   #%% Calculo SAR 
    ...
    '''Area del lazo'''
    Area_ciclo = abs(sc.integrate.trapz(magnetizacion_m_des,campo_m)) #Resultado negativo pude deberese al sentido antihorario del ciclo
    ...
    sar = mu_0*abs(Area_ciclo)*frecuencia_final_m/(1000*concentracion)  #multiplico por 1000 para pasar de concentracion en g/l a g/m^3
...
#%% Ciclo de Histereris final individual
    ...
    plt.plot([0,0],[Mr_mean/1000,-Mr_mean/1000],'D', label='$M_r$: %.2f A/m' %Mr_mean)
    plt.plot([Hc_mean/1000,-Hc_mean/1000],[0,0],'D', label='$H_c$: %.2f A/m' %Hc_mean)
...
#         #bbox=dict(boxstyle="round", alpha=0.7, lw=0.2), ha='right', va='center')
# 
# #plt.savefig(nombre_salida + ' - Ciclos de histéresis.png',dpi=500,bbox_inches='tight') 
# plt.show()
...
ascii.write(salida,'Resultados_Esar_'+ nombre_salida +'.txt',names=encabezado_salida,overwrite=True,delimiter='\t',formats=formato_salida)
